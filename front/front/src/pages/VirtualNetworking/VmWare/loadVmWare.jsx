...
const LoadVmWare = () => {
  ...
  const handleSubmit = async () => {
    ...
    // setIsError(false)
    try {
      const apiLocal = `http://localhost:8000/vmware?ip_address=${inputValues.ip}`;
      ...
      if (response.data) {
        setIsLoading(false);
        setData(response.data.vmwareList);
        setIsResult(true);
        // setIsSuccess(true)
      }
    } catch (error) {
